...
@onready var _new_game_button: Button    = $Center/ContentBox/NewGameButton
@onready var _quit_button:     Button    = $Center/ContentBox/QuitButton
@onready var _subtitle_label:  Label     = $Center/ContentBox/SubtitleLabel

func _ready() -> void:
	_style_button(_new_game_button, C_ACCENT_NEW)
	_style_button(_quit_button,     C_ACCENT_QUIT)
...
func _animate_subtitle() -> void:
	...
	tween.tween_property(_subtitle_label, "modulate:a", 0.55, 2.2).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(_subtitle_label, "modulate:a", 1.0,  2.2).set_ease(Tween.EASE_IN_OUT)
...
func _style_button(button: Button, accent: Color) -> void:
	...
	button.add_theme_color_override("font_color",          C_BTN_TEXT)
	button.add_theme_color_override("font_disabled_color", Color(C_BTN_TEXT, 0.35))
	button.add_theme_stylebox_override("normal",   _make_button_style(accent, C_BTN_BG))
	button.add_theme_stylebox_override("hover",    _make_button_style(accent, C_BTN_HOVER))
	button.add_theme_stylebox_override("pressed",  _make_button_style(accent, Color(accent, 0.25)))
	button.add_theme_stylebox_override("disabled", _make_button_style(Color(0.3, 0.3, 0.4), Color(0.06, 0.07, 0.10)))
	button.add_theme_stylebox_override("focus",    _make_button_style(accent, C_BTN_HOVER))
